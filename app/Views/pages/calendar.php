...
<style>
.month-pills{display:flex;gap:5px;flex-wrap:wrap;margin-bottom:16px;justify-content:center;}
...
@media(max-width:900px){.cal-main{padding:10px;}.cc{min-height:65px;}.dn{font-size:17px;}.cal-foot{grid-template-columns:1fr;}.fs{text-align:left;}}
@media(max-width:580px){.cc{min-height:44px;padding:2px 3px;}.dn{font-size:13px;}.bnd,.hpill{display:none;}.yr{font-size:38px;}}
</style>

<div class="month-pills">
    <?php for($m=1;$m<=12;$m++): ?>
    <a href="/calendar?month=<?=$m?>" class="mpill <?=$m===$viewMonth?'on':''?>"><?=$monthNames[$m]?></a>
    <?php endfor; ?>
</div>
<div class="nav-row">
    <a href="/calendar?month=<?=$prevMonth?>" class="nvbtn">← <?=$monthNames[$prevMonth]?></a>
    <div class="cur-lbl"><?=$monthName?> 2026</div>
    <a href="/calendar?month=<?=$nextMonth?>" class="nvbtn"><?=$monthNames[$nextMonth]?> →</a>
</div>
<div class="cal-card">
    <div class="mu-top">
...
document.addEventListener('click', e => {
    const pop = document.getElementById('popup');
    if (pop && !pop.contains(e.target) && !e.target.closest('.cc')) {
