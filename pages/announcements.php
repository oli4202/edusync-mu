...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $user['role'] === 'admin') {
    ...
    if ($action === 'post') {
        ...
        $sem     = (int)($_POST['target_semester'] ?? 0);
        $pinned  = isset($_POST['is_pinned']) ? 1 : 0;
        $expires = clean($_POST['expires_at'] ?? '');
        if (!$title || !$content) { $err = 'Title and content are required.'; }
        else {
            $db->prepare("INSERT INTO announcements (user_id,title,content,type,target_semester,is_pinned,expires_at) VALUES (?,?,?,?,?,?,?)")
               ->execute([$user['id'], $title, $content, $type, $sem, $pinned, $expires ?: null]);
...
$params = [];
// Non-admins see only their semester or all
if ($user['role'] !== 'admin') {
    $where[] = "(a.target_semester=0 OR a.target_semester=?)";
    $params[] = $user['semester'];
}

$annStmt = $db->prepare("SELECT a.*, u.name AS posted_by FROM announcements a
...
<title>Announcements — EduSync MU</title>
