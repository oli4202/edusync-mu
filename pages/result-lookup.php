...
header('Location: /result-lookup');
exit;

require_once __DIR__ . '/../includes/auth.php';
requireLogin();
$user = currentUser();
$currentPage = 'result-lookup';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>MU Result Lookup — EduSync MU</title>
...
.grade-A\+ { color:#34d399; font-weight:800; }
.grade-A  { color:#34d399; font-weight:800; }
.grade-A- { color:#34d399; font-weight:800; }
.grade-B\+ { color:#22d3ee; font-weight:800; }
.grade-B  { color:#22d3ee; font-weight:800; }
.grade-B- { color:#22d3ee; font-weight:800; }
.grade-C\+ { color:#fbbf24; font-weight:800; }
.grade-C  { color:#fbbf24; font-weight:800; }
.grade-D  { color:#f97316; font-weight:800; }
.grade-F  { color:#f87171; font-weight:800; }
...
.error-state { text-align:center; padding:24px; color:var(--danger); display:none; }
...
<main class="main">
    ...
    <div class="result-hero">
        ...
        <a href="https://metrouni.edu.bd/sites/department-of-software-engineering/result-se" target="_blank"
           class="btn btn-outline" style="margin-top:16px;font-size:12px;">🌐 Open Official MU Website</a>
    </div>
    ...
    <div class="result-table-wrap" id="resultWrap">
        ...
        <div style="margin-top:16px;font-size:12px;color:var(--muted);text-align:center;padding:12px;border:1px solid var(--border);border-radius:8px;">
            ℹ️ This result is fetched live from Metropolitan University Sylhet official servers. For any discrepancy, contact the Examination Controller's Office.
        </div>
    </div>
</main>
...
async function searchResult() {
    ...
    try {
        const resp = await fetch('ajax/mu-result.php', {
            method: 'POST',
        ...
        (data.results || []).forEach(r => {
            const gradeClass = 'grade-' + (r.grade || '').replace('+','\\+').replace('-','\\-');
            tbody.innerHTML += `<tr>
                <td><strong>${r.code || '—'}</strong></td>
                <td>${r.title || '—'}</td>
                <td style="text-align:center">${r.credit || '—'}</td>
                <td style="text-align:center">${r.marks || '—'}</td>
                <td style="text-align:center;font-weight:800;color:${gradeColor(r.grade)}">${r.grade || '—'}</td>
                <td style="text-align:center">${r.gp || '—'}</td>
                <td style="text-align:center"><span class="badge ${r.status==='Pass'?'badge-green':'badge-red'}">${r.status||'—'}</span></td>
            </tr>`;
            if(r.gp && r.credit) { totalGP += r.gp*r.credit; totalCr += parseFloat(r.credit); }
        });
